...
from KvChat.models.qwen3 import Qwen3ForCausalLM
from KvChat.layers.sampler import Sampler
from KvChat.utils.context import set_context, get_context, reset_context
from KvChat.utils.loader import load_model
from KvChat.engine.query_window_manager import QueryWindowManager
...
class ModelRunner:
    ...
    def prepare_decode(self, seqs: list[Sequence]):
        ...
        if self.kv_compress_enabled:
            self.prepare_compression_context(seqs)
    ...
    def set_compress_enabled(self, enabled: bool):
        self.config.kv_compress_enabled = enabled
        self.kv_compress_enabled = enabled
        for module in self.model.modules():
            if hasattr(module, "kv_compress_enabled"):
                module.kv_compress_enabled = enabled
    def prepare_compression_context(self,seqs):
        ctx = get_context()
        ...
        for i, seq in enumerate(seqs):
            ...
            S_i = seq.num_prompt_tokens + self.config.kv_compress_S
            R_i = seq.num_prompt_tokens + self.config.kv_compress_R
            if runtime_S == 0:
                runtime_S = S_i
                runtime_R = R_i
            L = len(seq)
            need_compress = (L == S_i)
            compress_need_mask.append(need_compress)
            active = (L > S_i - W) and (L <= S_i)
            if active:
        ...
        ctx.compress_need_mask = compress_need_mask
        ctx.compress_any = any(compress_need_mask)

        ctx.q_window_active_indices = q_window_active_indices
        ctx.q_window_active_seq_ids = q_window_active_seq_ids
        ctx.seq_ids = seq_ids

        ctx.runtime_compress_S = runtime_S
        ctx.runtime_compress_R = runtime_R